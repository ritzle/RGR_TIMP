...
from utils import generate_code, send_email, decode_token  # Добавлен decode_token
import bcrypt
import time
...
def validate_token_and_email(email_from_request):
    """Общая функция проверки токена и email"""
    auth_header = request.headers.get("Authorization")
    if not auth_header or not auth_header.startswith("Bearer "):
        logger.warning("❌ Отсутствует или неверный токен авторизации")
        return None, jsonify({"message": "Неавторизованный доступ"}), 401

    token = auth_header.split(" ")[1]
    user_data = decode_token(token)
    if not user_data:
        logger.warning("❌ Невалидный токен")
        return None, jsonify({"message": "Невалидный токен"}), 401

    token_email = user_data.get("email").lower()
    if token_email != email_from_request.lower():
        logger.warning(f"❌ Несовпадение email в токене и запросе: {email_from_request} != {token_email}")
        return None, jsonify({"message": "Доступ запрещён"}), 403

    return token_email, None, None
...
def send_reset_code():
    try:
        ...
        # Проверка токена
        _, error_response, status_code = validate_token_and_email(email)
        if error_response:
        ...
        logger.error(f"❌ Ошибка при отправке кода сброса: {e}")
